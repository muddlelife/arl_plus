...
class InfoHunter(object):
    ...
    def __init__(self, sites: list):
        ...
        # Prefer the bundled binary if present; fall back to PATH.
        bundled = os.path.join(os.path.dirname(__file__), '..', 'tools', 'wih')
        bundled = os.path.abspath(bundled)
        self.wih_bin_path = bundled if os.path.isfile(bundled) else "wih"
